...
simpsons.forEach(item => {
    ...
    for (let key in item) {
        if (key !== `photo`) {
            ...
            div_element.innerText = `${key}: ${item[key]}`
...
coursesArray.forEach(item => {
    ...
    duration_div.append(month_div, hours_div)
    duration_div.style.display = `flex`
    month_div.style.width = `25%`
    hours_div.style.width = `75%`
    const modules_div = document.createElement(`div`)
    title_div.innerText = item.title
    hours_div.innerText = item.hourDuration
    month_div.innerText = item.monthDuration
    item.modules.forEach(module => {
        const module_div = document.createElement(`div`)
        module_div.innerText = module
        modules_div.appendChild(module_div)
    })
    item_div.append(title_div, duration_div, modules_div)
